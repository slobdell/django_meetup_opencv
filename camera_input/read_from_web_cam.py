...
IP_ADDRESS = "10.10.100.45"
PORT = 80

# authentication requirements have been removed from the camera
CAMERA_URL = "http://%s:%s/mjpeg.cgi" % (IP_ADDRESS, PORT)

# HACKY!  The bytes sent do not match content length but they generally match
# about 0.93 through 0.96 if the content-length.  This gives us a minor
# performance boost that we need to speed this up
EXAMINE_THRESHOLD = 0.9
...
def image_yielder(response_stream):
    ...
    prev_char = " "
    for line in response_stream.iter_content(chunk_size=512, decode_unicode=False):
        if current_state == StreamState.SEEKING_META_DATA:
            ...
            for byte in line:
                bytes_examined += 1
                percent_read = float(bytes_examined) / content_length
                if percent_read >= EXAMINE_THRESHOLD:
                    ...
                    if current_char == "d9" and prev_char == "ff":
                        ...
                        # img_array = cv2.imdecode(np.asarray(raw_bytes), cv2.CV_LOAD_IMAGE_GRAYSCALE)
                        img_array = cv2.imdecode(np.asarray(raw_bytes), cv2.CV_LOAD_IMAGE_COLOR)
                        yield img_array
                        meta_data_seeker = StringIO()
                        current_state = StreamState.SEEKING_META_DATA
                        image_buffer = None
...
def get_cv_img_from_ip_cam():
    while True:
        try:
            ...
            print "ERROR!  %s.  Waiting 10 seconds and retrying...." % e
            time.sleep(10)
